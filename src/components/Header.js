...
function Header() {
  ...
  const [loginClassName, setLoginClassName] = useState("header__auth-container header__auth-container_hidden");
  ...
  useEffect(()=>{
    if (visibilityLoginData) {
      ...
      setLoginClassName("header__auth-container header__auth-container_logged header__auth-container_hidden");
      ...
      <header className={headerClassName}>
        <img className="logo logo_header" src={logo} alt="Логотип" />
          <div className={loginClassName}>
            ...
            <Link to='/sign-in' className='header__link-auth header__link-auth_logged' onClick={logOut}>
                  Выйти
            </Link>
          </div>
          <button type="button" className={buttonClassName} onClick={changeVisibilityLoginData}/>
      </header>
    </Route>
  </Switch>
